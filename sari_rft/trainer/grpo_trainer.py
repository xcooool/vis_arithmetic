...
class Qwen2VLGRPOTrainer(Trainer):
    ...
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        ...
        images = []
        for x in inputs:
            images.append(x["image1"])
            images.append(x["image2"])
            images.append(x["image3"])
        ...
        with unwrap_model_for_generation(model, self.accelerator) as unwrapped_model:
            ...
            # completions = self.processing_class.batch_decode(prompt_completion_ids, skip_special_tokens=True)
            # if is_conversational(inputs[0]):
            #     completions = [[{"role": "assistant", "content": completion}] for completion in completions]

            # # Compute the rewards
            # prompts = [prompt for prompt in prompts for _ in range(self.num_generations)]
            # print("before decode prompts: ", prompts)
            # print("before decode completions: ", completions)
            ...
            # # Compute the rewards
            # prompts = [prompt for prompt in prompts for _ in range(self.num_generations)]
            # print("after decode prompts: ", prompts)
            # print("after decode completions: ", completions)
        ...
        # 这里计算好了所有的reward，进行加权
        # Sum the rewards from all reward functions
        # lambda_ = 0.98
        # weights = torch.tensor([0.7, 0.3], device=rewards_per_func.device)
